...
DATASET = "tabular_covertype"
URL = ("https://archive.ics.uci.edu/ml/machine-learning-databases/"
        "covtype/covtype.data.gz")


def load_covertype(seed: int):
    import numpy as np
    import pandas as pd
    dest = CACHE_DIR / "covtype.data.gz"
    try:
        if not dest.exists():
        ...
        y = df.iloc[:, -1].to_numpy(dtype=np.int64) - 1
        num_classes = int(y.max()) + 1
        X_t = torch.from_numpy(X)
        y_t = torch.from_numpy(y)
        sample = min(50_000, X_t.shape[0])
        ...
        print(f"[fallback] covertype download failed ({exc}); "
...
def main():
    ...
    set_seed(args.seed)
    Xtr, ytr, Xte, yte, num_classes = load_covertype(args.seed)
